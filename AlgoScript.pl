...
sub MakeSUB{
	...
	return MakeSUB_OR               ($AST) if(AST::EstOR               ($AST));
	return MakeSUB_AND              ($AST) if(AST::EstAND              ($AST));
	return MakeSUB_XOR              ($AST) if(AST::EstXOR              ($AST));
	return MakeSUB_NOT              ($AST) if(AST::EstNOT              ($AST));
	return MakeSUB_ADD              ($AST) if(AST::EstADD              ($AST));
	return MakeSUB_SUBS             ($AST) if(AST::EstSUBS             ($AST));
	return MakeSUB_MULT             ($AST) if(AST::EstMULT             ($AST));
	return MakeSUB_DIV              ($AST) if(AST::EstDIV              ($AST));
	return MakeSUB_CONCATENE        ($AST) if(AST::EstCONCATENE        ($AST));
	return MakeSUB_VRAI             ($AST) if(AST::EstVRAI             ($AST));
	return MakeSUB_FAUX             ($AST) if(AST::EstFAUX             ($AST));
	return MakeSUB_VOID             ($AST) if(AST::EstVOID             ($AST));
	return MakeSUB_NOTHING          ($AST) if(AST::EstNOTHING          ($AST));
	return MakeSUB_ASSIGN           ($AST) if(AST::EstASSIGN           ($AST));
	return MakeSUB_AFFECT           ($AST) if(AST::EstAFFECT           ($AST));
	return MakeSUB_ID               ($AST) if(AST::EstID               ($AST));
	return MakeSUB_LIST_INSTRUCTIONS($AST) if(AST::EstLIST_INSTRUCTIONS($AST));
	return MakeSUB_NUMERIC          ($AST) if(AST::EstNUMERIC          ($AST));
	return MakeSUB_CONDITION        ($AST) if(AST::EstCONDITION        ($AST));
	return MakeSUB_TANTQUE          ($AST) if(AST::EstTANTQUE          ($AST));
	return MakeSUB_EQUALITE         ($AST) if(AST::EstEQUALITE         ($AST));
	return MakeSUB_STRICTSUP        ($AST) if(AST::EstSTRICTSUP        ($AST));
	return MakeSUB_AFFICHE          ($AST) if(AST::EstAFFICHE          ($AST));
	return MakeSUB_STRING           ($AST) if(AST::EstSTRING           ($AST));
	return MakeSUB_STRINGQUOTE      ($AST) if(AST::EstSTRINGQUOTE      ($AST));
	return MakeSUB_BOUCLEPOUR       ($AST) if(AST::EstBOUCLEPOUR       ($AST));
	return MakeSUB_POURCONTEXT      ($AST) if(AST::EstPOURCONTEXT      ($AST));
	return MakeSUB_POURCHAQUE       ($AST) if(AST::EstPOURCHAQUE       ($AST));
	return MakeSUB_FUNCASSIGN       ($AST) if(AST::EstFUNCASSIGN       ($AST));
	return MakeSUB_LAMBDA           ($AST) if(AST::EstLAMBDA           ($AST));
	return MakeSUB_INTERNALFUNC     ($AST) if(AST::EstINTERNALFUNC     ($AST));
	return MakeSUB_DEFLAMBDA        ($AST) if(AST::EstDEFLAMBDA        ($AST));
	return MakeSUB_DEFRANGE         ($AST) if(AST::EstDEFRANGE         ($AST));
	return MakeSUB_DEFTABLO         ($AST) if(AST::EstDEFTABLO         ($AST));
	return MakeSUB_TABLO            ($AST) if(AST::EstTABLO            ($AST));
	return MakeSUB_BRACKET          ($AST) if(AST::EstBRACKET          ($AST));
	return MakeSUB_INPUT            ($AST) if(AST::EstINPUT            ($AST));
	return MakeSUB_ERROR            ($AST) if(AST::EstERROR            ($AST));
	return MakeSUB_INSPECT          ($AST) if(AST::EstINSPECT          ($AST));
